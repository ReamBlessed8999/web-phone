...
// 1. បន្ថែម ORDER_STATUSES ការពារ Error ពេល Render Table
const ORDER_STATUSES = ['Pending', 'Processing', 'Shipped', 'Delivered', 'Cancelled'];

// 2. អនុគមន៍ការពារ Security មិនឲ្យ User ធម្មតា ឬ Guest ចូល Admin Page
...
function renderDashboardStats() {
  try {
    ...
    const completedOrdersList = orders.filter(o => o && o.status === 'Delivered');
    const pendingOrdersList = orders.filter(o => o && o.status === 'Pending');
    ...
    setEl('statTotalSales', typeof formatPrice === 'function' ? formatPrice(totalSales) : '$' + totalSales);
    ...
    console.error("Error rendering dashboard stats:", err);
  }
}
...
function renderSalesStatistics() {
  try {
    ...
    const completedOrders = orders.filter(o => o && o.status === 'Delivered');
    ...
    const setEl = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val; };
    setEl('salesTotalRevenue', typeof formatPrice === 'function' ? formatPrice(totalRevenue) : '$' + totalRevenue);
    ...
    console.error("Error rendering sales statistics:", err);
  }
}

/* ============================================================
   PRODUCT CRUD & DELETE CONFIRMATION
   ============================================================ */
...
function renderAdminOrdersTable() {
  ...
  const orders = typeof loadOrders === 'function' ? loadOrders().sort((a, b) => b.date - a.date) : [];
  if (!orders.length) {
